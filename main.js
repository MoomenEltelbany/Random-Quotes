...
let changeBg = () => {
    ...
    quoteBox.style.cssText = `background-color: rgb(${Math.floor(
        Math.random() * red
    )}, ${Math.floor(Math.random() * green)}, ${Math.floor(
        Math.random() * blue
    )} )`;
};
...
let randomQuote = () => {
    ...
    quoteBlock.innerHTML = quotes[randomIndex].quote;
    citation.innerHTML = quotes[randomIndex].citation || "";
    citation.innerHTML = quotes[randomIndex].source;
    year.innerHTML = quotes[randomIndex].year || "";
